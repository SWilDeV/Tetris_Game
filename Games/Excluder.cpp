...
Vect2D Excluder::GetCollisionOffset(const AARectangle& rect) const
{
	BoundaryEdge edge;
	Vect2D offset = Vect2D::Zero;
...
void Excluder::MoveBy(const Vect2D& delta)
{
	mAARect.MoveBy(delta);
	SetupEdges();
}
void Excluder::MoveTo(const Vect2D& point)
{
