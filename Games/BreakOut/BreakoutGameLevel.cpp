...
void BreakoutGameLevel::Update(uint32_t dt, Ball& ball)
{
	...
	for (auto& block : mBlocks)
	{
		...
		if (!block.IsDestroyed() && block.HasCollided(ball.GetBoundingRect(), edge))
		{
			...
			mPoints += 10;
			std::cout << "mPoints: " << mPoints << std::endl;
		}
	}
...
void BreakoutGameLevel::CreateDefaultLevel(const AARectangle& boundary)
{
	...
	{
		AARectangle blockRect = { Vect2D(startX, BLOCK_HEIGHT * (r + 1)), BLOCK_WIDTH, BLOCK_HEIGHT };
		...
		for (int c = 0; c < NUM_BLOCKS_ACROSS; ++c)
		{
			...
			mBlocks.push_back(b);
			blockRect.MoveBy(Vect2D(BLOCK_WIDTH, 0));
		}
	}
}
...
std::vector<BreakoutGameLevel> BreakoutGameLevel::LoadLevelsFromFile(const std::string& filePath)
{
	...
	levelCommand.parseFunc = [&](ParseFuncParams)
	{
		if (levels.size() > 0)
		{
		...
		level.Init(AARectangle(Vect2D::Zero, App::Singleton().Width(), App::Singleton().Heigth()));
	...
	layoutCommand.parseFunc = [&](ParseFuncParams params)
	{
		...
		float startingX = 0; 
		AARectangle blockRect(Vect2D(startingX, (params.lineNum + 1) * BLOCK_HEIGHT), BLOCK_WIDTH, BLOCK_HEIGHT);
