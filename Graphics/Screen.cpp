...
Screen::Screen() : mWidth(0), mHeigth(0), moptrWindow(nullptr), mnoptrWindowSurface(nullptr)
{
}
Screen ::~Screen()
{
...
SDL_Window* Screen:: Init(uint32_t w, uint32_t h, uint32_t mag)
{
	...
	{
		std::cout << "SDL could not be initatilized! SDL Error" << SDL_GetError() << std::endl;
		return nullptr;
	}
	mWidth = w;
	mHeigth = h;

	// Create a mini Window and store it in a SDL_Window pointer object;
	moptrWindow = SDL_CreateWindow("Arcade", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, mWidth*mag, mHeigth*mag, 0);

	if (moptrWindow)
	{
		//Access the surface of the mini window
		mnoptrWindowSurface = SDL_GetWindowSurface(moptrWindow);

		// say that the pixel format is coming from noptrwindowsurface
		SDL_PixelFormat* pixelFormat = SDL_AllocFormat(SDL_PIXELFORMAT_RGBA8888); // change from: " = mnoptrWindowSurface->format" that was only RGB8888

		Color::InitColorFormat(pixelFormat);

		mClearColor = Color::Black();

		mBackBuffer.Init(pixelFormat->format, mWidth, mHeigth);
...
void Screen::Draw(const Vect2D point, const Color& color)
{
	assert(moptrWindow);
	if (moptrWindow)
	{
...
void Screen::Draw(const Star& star, const Color& color, bool fill, const Color& fillColor)
{
	...
	Line2D line1 = Line2D(star.GetP2(), star.GetP4());
	Line2D line2 = Line2D(star.GetP2(), star.GetP5());
	Line2D line3 = Line2D(star.GetP6(), star.GetP3());
	Line2D line4 = Line2D(star.GetP6(), star.GetP4());
	Line2D line5 = Line2D(star.GetP3(), star.GetP5());
...
void Screen::Draw(const AARectangle& rect, const Color& color, bool fill, const Color& fillColor)
{
	...
	std::vector<Vect2D> points = rect.GetPoints();
...
void Screen::Draw(const Circle& circle, const Color& color, bool fill, const Color& fillColor)
{
	...
	static unsigned int NUM_CIRCLE_SEGMENTS = 30.0f;

	std::vector<Vect2D> circlePoints;
	std::vector<Line2D> lines;

	float angle = TWO_PI / float(NUM_CIRCLE_SEGMENTS);

	Vect2D p0 = Vect2D(circle.GetCenterPoint().GetX() + circle.GetRadius(), circle.GetCenterPoint().GetY());
	Vect2D p1 = p0;
	Line2D nextLineToDraw;
	Vect2D centerpoint = circle.GetCenterPoint();

	for (unsigned int i = 0; i < NUM_CIRCLE_SEGMENTS; ++i)
	{
		p1.Rotate(angle, centerpoint);
...
void Screen::FillPoly(const std::vector<Vect2D>& points, const Color& color)
{
	if (points.size() > 0)
	{
